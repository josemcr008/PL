...
/*!
  \class CadNode
  \brief Definition of atributes and methods of CadNode class
  \note  CadNode Class publicly inherits from ExpNode class
*/
...
class Plus_unoNode : public NumericUnaryOperatorNode
{
protected:
  ExpNode *_num;
...
/*!
	...
	\post  A new UnaryPlusNode is created with the parameter
*/
  Plus_unoNode(ExpNode *num): NumericUnaryOperatorNode(num)
	{
    this->_num=num;
...
/*!
	...
	\post  A new EmptyStmt is created
*/
  LugarStmt(ExpNode* exp1, ExpNode *exp2)
	{
    this->_exp1= exp1;
    this->_exp2=exp2;
		// Empty
...
{
 private:
  std::string _id; //!< Condicion of the if statement
  ExpNode *_from; //!< Condicion of the if statement
  ExpNode *_until; //!< Condicion of the if statement
  ExpNode *_pass; //!< Condicion of the if statement
