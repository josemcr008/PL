...
%type <st> stmt asgn print read read_cad if while repetir para casos borrar lugar printcad
...
%token PRINT READ IF THEN ELSE END_IF WHILE DO END_WHILE REPETIR UNTIL FOR FROM PASS END_FOR CASE VALUE END_CASE LUGAR CASOS VALOR DEFECTO FIN_CASOS BORRAR COLON PRINTCAD READ_CAD
...
%left PLUS MINUS CONCAT PLUS_UNO
...
repetir: REPETIR controlSymbol stmtlist UNTIL  cond
		{
...
lugar:  LUGAR controlSymbol LPAREN exp COMMA exp RPAREN
  {
    $$ = new lp::LugarStmt($4, $6);
    control--;
  }
;
...
;
read_cad:	READ_CAD LPAREN VARIABLE RPAREN
		{
			// Create a new read node
